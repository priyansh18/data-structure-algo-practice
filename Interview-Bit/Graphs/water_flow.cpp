...
int solve(vector<vector<int>> &A, int n11, int n12) {
    queue<ValPair> q1;
    queue<ValPair> q2;
    ...
    while (!q1.empty()) {
        ...
        for (int k = 0; k < 4; k++) {
            ...
            while (newDx >= 0 and newDy >= 0 and newDx < n11 and newDy < n12 and
                   !blue_lake[newDx][newDy] and A[newDx][newDy] >= temp.val) {
    ...
    while (!q2.empty()) {
        ...
        for (int k = 0; k < 4; k++) {
            ...
            while (newDx >= 0 and newDy >= 0 and newDx < n11 and newDy < n12 and
                   !red_lake[newDx][newDy] and A[newDx][newDy] >= temp.val) {
    ...
    for (int i = 0; i < n11; i++) {
        for (int j = 0; j < n12; j++) {
            if (blue_lake[i][j] == red_lake[i][j]) count++;
        }
    }
